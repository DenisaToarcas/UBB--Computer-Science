...
void UI::addTutorialUI() {
    ...
    if (!validateString(new_title) || (new_title.length() == 0))
        throw "Invalid title!";

    std::cout<<"Presenter: "<<std::endl;
    getline(std::cin, new_presenter);
    if (!validateString(new_presenter) || (new_presenter.length() == 0))
        throw "Invalid presenter!";
    ...
    if (validateString(minutes_string) == 0 && minutes_string.length() != 0)
        ...
        throw "Invalid input for minutes!";
    ...
    if (validateString(seconds_string) == 0 && seconds_string.length() != 0)
        ...
        throw "Invalid input for seconds!";

    if (seconds > 60)
        throw "Time defined wrongly!";
    else
    ...
    additionSuccessful = this->service->addTutorialService(new_title, new_presenter, new_duration, new_likes, new_link);
    if (!additionSuccessful)
        throw "Tutorial is already part of the list!";
    else
...
void UI::deleteTutorialUI() {
    ...
    bool deleted = false;
    deleted = this->service->deleteTutorialService(tutorial_to_delete);
    if (!deleted)
        throw "Tutorial is not part of the list!";
    else
...
void UI::updateTutorialUI() {
    ...
    updated = this->service->updateTutorialService(title, new_presenter, new_duration, new_likes, new_link);
    if (!updated)
        throw "Tutorial is not part of the list!";
    else
...
void UI::listTutorialsUI() {
    if (this->service->getSizeService() == 0)
        throw "There are no tutorials in the list!";
    else
    {
        std::cout<<"The list of tutorials is: "<<std::endl;
        for (int i=0; i< this->service->getSizeService(); i++)
            std::cout<<this->service->getTutorialService(i).toString()<<std::endl;
    }
}
...
void UI::tutorials_with_given_presenter() {
    ...
    if (!validateString(presenter) || presenter.length() == 0)
        throw "Invalid name for the presenter!";
    ...
    while (show_next == "next") {
        ...
        if (option_add_tutorial == "1") {
            ...
            added = this->service->addTutorialWatchListService(_title, _presenter, _duration, _likes, _link);

            if (!added)
                throw "Tutorial was already added to the watch list!";
...
void UI::delete_from_watch_list()
{
    ...
    bool deleted = false;
    deleted = this->service->deleteTutorialWatchListService(tutorial_to_delete);
    if (!deleted)
        throw "Tutorial is not part of the list!";
    else {
        std::cout << "Tutorial deleted successfully!" << std::endl;
...
void UI::see_watch_list()
{
    if (this->service->getSizeWatchList() == 0)
        throw "There are no tutorials in the watch list!";
    else
    {
        std::cout<<"The watch list is: "<<std::endl;
        for (int i=0; i< this->service->getSizeWatchList(); i++)
            std::cout<<this->service->getTutorialWatchList(i).toString()<<std::endl;
    }
}
