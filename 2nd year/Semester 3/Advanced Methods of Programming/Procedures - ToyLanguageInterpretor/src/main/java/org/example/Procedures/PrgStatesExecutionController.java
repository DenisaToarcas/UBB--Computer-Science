...
public class PrgStatesExecutionController implements Initializable {
    ...
    //--------PROCEDURE-------------------------------
    @FXML
    private TableView<Map.Entry<String, javafx.util.Pair<List<String>, IStmt>>> ProcedureTableView;

    @FXML
    private TableColumn<Map.Entry<String, javafx.util.Pair<List<String>, IStmt>>, javafx.util.Pair<String, List<String>>> ProcNameAndVarColumn;

    @FXML
    private TableColumn<Map.Entry<String, javafx.util.Pair<List<String>, IStmt>>, String> ProcBodyColumn;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        for (IStmt stmt: stmts)
        {
            ...
            MyIStack<MyIDictionary<String, Value>> SymTbl = new MyStack<MyIDictionary<String, Value>>();
            ...
            try {
                ...
                PrgState prgState = new PrgState(exeStack, SymTbl, Out, FileTable, Heap, procTable, stmt);
        ...
        ProcNameAndVarColumn.setCellValueFactory(p ->
        {
            return new SimpleObjectProperty<>(new javafx.util.Pair<String, List<String>>(p.getValue().getKey(), p.getValue().getValue().getKey()));
        });
        ProcBodyColumn.setCellValueFactory(p ->
        {
            return new SimpleStringProperty(p.getValue().getValue().getValue().toString());
        });
    }
    ...
    public void populateSymTableTableView()
    {
        ...
        ArrayList<Pair<String, String>> symbolTableEntries = new ArrayList<>();
        if (symbolTable != null && !symbolTable.isEmpty()) {
            for (Map.Entry<String, Value> entry : symbolTable.getContent().entrySet()) {
                symbolTableEntries.add(new Pair<>(entry.getKey(), entry.getValue().toString()));
            }
        }
        SymTableTableView.setItems(FXCollections.observableArrayList(symbolTableEntries));
    }
    ...
    {
        MyIProcTable procTable = Objects.requireNonNull(getPrg()).getProcTable();
        ...
        ProcedureTableView.setItems(FXCollections.observableList(procTableList));
    }
    ...
    public void addAllStatements()
    {
        ...
        IStmt procSum = new CompStmt(new VarDeclStmt("v", new IntType()),
                new CompStmt(new AssignStmt("v", new ArithExp("+", new VarExp("a"), new VarExp("b"))),
                        new PrintStmt(new VarExp("v"))));
        List<String> varSum = new ArrayList<>();
        varSum.add("a");
        varSum.add("b");
        procTable.add("sum", new Pair<>(varSum, procSum));

        IStmt procProd = new CompStmt(new VarDeclStmt("v", new IntType()),
                new CompStmt(new AssignStmt("v", new ArithExp("*", new VarExp("a"), new VarExp("b"))),
                        new PrintStmt(new VarExp("v"))));
        List<String> varProd = new ArrayList<>();
        varProd.add("a");
        varProd.add("b");
        procTable.add("prod", new Pair<>(varProd, procProd));

        IStmt exProc = new CompStmt(new VarDeclStmt("v", new IntType()),
                new CompStmt(new AssignStmt("v", new ValueExp(new IntValue(2))),
                        new CompStmt(new VarDeclStmt("w", new IntType()),
                                new CompStmt(new AssignStmt("w", new ValueExp(new IntValue(5))),
                                        new CompStmt(new callProcedure("sum", new ArrayList<>(Arrays.asList(new ArithExp("*", new VarExp("v"), new ValueExp(new IntValue(10))), new VarExp("w")))),
                                                new CompStmt(new PrintStmt(new VarExp("v")),
                                                        new ForkStmt(new CompStmt(
                                                                new callProcedure("prod", new ArrayList<>(Arrays.asList(new VarExp("v"), new VarExp("w")))),
                                                                new ForkStmt(
