package org.example.toySemaphore;
...
public class PrgStatesExecutionController implements Initializable {
    ...
    @FXML
    private TableView<Map.Entry<Integer, model.adts.Pair<Integer, List<Integer>>>> ToySemaphoreTableView;

    @FXML
    private TableColumn<Map.Entry<Integer, model.adts.Pair<Integer, List<Integer>>>, Integer> ValueColumn;

    @FXML
    private TableColumn<Map.Entry<Integer, model.adts.Pair<Integer, List<Integer>>>, Integer> IndexColumn;

    @FXML
    private TableColumn<Map.Entry<Integer, model.adts.Pair<Integer, List<Integer>>>, List<Integer>> ListValuesColumn;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        for (IStmt stmt: stmts)
        {
            ...
            MyISemaphore toySemaphore = new MySemaphore();

            try {
                stmt.typeCheck(typeEnv);
                PrgState prgState = new PrgState(exeStack, SymTbl, Out, FileTable, Heap, toySemaphore, stmt);
        ...
        {
            return new SimpleIntegerProperty(p.getValue().getValue().getFirst()).asObject();
        });
        ListValuesColumn.setCellValueFactory(p ->
        {
            return new SimpleObjectProperty<>(p.getValue().getValue().getSecond());
        });
    }
    ...
    public void populateToySemaphoreTableView()
    {
        PrgState programState = getPrg();
        if (programState != null) {
            MyISemaphore semaphoreTable = Objects.requireNonNull(programState).getSemaphore();
            List<Map.Entry<Integer, model.adts.Pair<Integer, List<Integer>>>> toySemaphoreList = new ArrayList<>();
            toySemaphoreList.addAll(semaphoreTable.getToySemaphore().entrySet());
    ...
    public void addAllStatements()
    {
        ...
        //TOY SEMAPHORE
        IStmt exSem = new CompStmt(new VarDeclStmt("v1", new RefType(new IntType())),
                new CompStmt(new VarDeclStmt("cnt", new IntType()),
                        new CompStmt(new aHStmt("v1", new ValueExp(new IntValue(1))),
                                new CompStmt(new createSemaphore("cnt", new rHExp(new VarExp("v1"))),
                                        new CompStmt(new ForkStmt(
                                                new CompStmt(new AcquireStmt("cnt"), new CompStmt(new wHStmt("v1", new ArithExp("*", new rHExp(new VarExp("v1")), new ValueExp(new IntValue(10)))),
                                                        new CompStmt(new PrintStmt(new rHExp(new VarExp("v1"))), new ReleaseStmt("cnt"))))),
                                                new CompStmt(new ForkStmt(
                                                        new CompStmt(new AcquireStmt("cnt"), new CompStmt(new wHStmt("v1", new ArithExp("*", new rHExp(new VarExp("v1")), new ValueExp(new IntValue(10)))),
