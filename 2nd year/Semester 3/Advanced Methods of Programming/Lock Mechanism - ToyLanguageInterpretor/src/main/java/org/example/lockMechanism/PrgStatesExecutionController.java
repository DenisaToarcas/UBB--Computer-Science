...
public class PrgStatesExecutionController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        for (IStmt stmt: stmts)
        {
            ...
            try {
                ...
                PrgState prgState = new PrgState(exeStack, SymTbl, Out, FileTable, Heap, lockTable, stmt);
    ...
    public void populateLockTableView()
    {
        ...
        if (programState != null) {
            MyILock lockTable = Objects.requireNonNull(programState).getLock();
            ArrayList<Pair<Integer, Integer>> lockTableEntries = new ArrayList<>();
            for (Map.Entry<Integer, Integer> entry : lockTable.getLock().entrySet()) {
    ...
    public void populatePrgStateIdentifiersListView()
    {
        if (controller != null) {
            ...
            for (PrgState state : programStates)
                if (!prgStates.contains(state)) {
                    prgStates.add(state);
                    this.prgStateIdentifiers.add(String.valueOf(state.getPrgStateID()));
                }
        }
        this.PrgStateIdentifiersListView.setItems(prgStateIdentifiers);
        NoOfPrgStatesTextField.setText(String.valueOf(prgStateIdentifiers.size()));
    }
    ...
    public void checkIfPrgStateIdentifierSelected()
    {
        ...
        List<Pair<String, String>> symTbl = new ArrayList<>();
        for (String key : this.prgStates.get(PrgStateIdentifierSelected).getSymTable().getContent().keySet())
        {
            symTbl.add(new Pair<>(key, this.prgStates.get(PrgStateIdentifierSelected).getSymTable().getValue(key).toString()));
        }

        this.SymTableTableView.setItems(FXCollections.observableList(symTbl));

        List<Pair<String, String>> heapTbl = new ArrayList<>();
        for (Integer key : this.prgStates.get(PrgStateIdentifierSelected).getHeap().getContent().keySet())
        {
                heapTbl.add(new Pair<>(key.toString(), this.prgStates.get(PrgStateIdentifierSelected).getHeap().getValue(key).toString()));
        }

        this.HeapTableView.setItems(FXCollections.observableList(heapTbl));

        List<String> outList = Collections.singletonList(this.prgStates.get(PrgStateIdentifierSelected).getOut().toString());
        this.OutListView.setItems(FXCollections.observableList(outList));

        if (this.prgStates.get(PrgStateIdentifierSelected) != null) {
            MyILock lockTable = Objects.requireNonNull(this.prgStates.get(PrgStateIdentifierSelected)).getLock();
            ArrayList<Pair<Integer, Integer>> lockTableEntries = new ArrayList<>();
            for (Map.Entry<Integer, Integer> entry : lockTable.getLock().entrySet()) {
                lockTableEntries.add(new Pair<>(entry.getKey(), entry.getValue()));
            }
